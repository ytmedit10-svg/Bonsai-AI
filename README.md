...
GEMINI_AVAILABLE_MODELS=gemma-4-e4b-it,gemma-4-31b-it
GEMINI_DEFAULT_MODEL=gemma-4-31b-it
GEMINI_MERGE_MODEL=gemma-4-31b-it
...
Keep hosted model names limited to the Gemma 4 models you want users to access.
The frontend does not hardcode hosted model choices; it renders the list returned
by the API.
...
| `GEMINI_AVAILABLE_MODELS` | Comma-separated hosted Gemma 4 models shown in the UI |
...
- Hosted Gemini mode: confirm `AI_PROVIDER=gemini` and
  `GEMINI_AVAILABLE_MODELS` contains comma-separated Gemma 4 model names.
